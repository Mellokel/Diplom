...
open class AlamofireRequestBuilder<T>: RequestBuilder<T> {
    ...
    override open func execute(_ completion: @escaping (_ response: Response<T>?, _ error: Error?) -> Void) {
        ...
        SwaggerAPI.getToken { (header, error) in
            self.addHeaders(header)
        
        let managerId:String = UUID().uuidString
        // Create a new manager for each request to customize its request header
        let manager = self.createSessionManager()
        managerStore[managerId] = manager

        let encoding:ParameterEncoding = self.isBody ? JSONDataEncoding() : URLEncoding()

        let xMethod = Alamofire.HTTPMethod(rawValue: self.method)
        let fileKeys = self.parameters == nil ? [] : self.parameters!.filter { $1 is NSURL }
                                                           .map { $0.0 }
        ...
        if fileKeys.count > 0 {
            manager.upload(multipartFormData: { mpForm in
                ...
                }, to: self.URLString, method: xMethod!, headers: nil, encodingCompletion: { encodingResult in
            ...
            let request = self.makeRequest(manager: manager, method: xMethod!, encoding: encoding, headers: self.headers)
            if let onProgressReady = self.onProgressReady {
